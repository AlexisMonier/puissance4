...
def evaluate_window(window, piece):
	...
	if window.count(opp_piece) == 4 and window.count(EMPTY) == 1:
		score -= 15
	if window.count(opp_piece) == 3 and window.count(EMPTY) == 2:
		score -= 8
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.MOUSEBUTTONDOWN:
			...
			if turn == PLAYER:
				...
				# col = random.randint(0,6) Avec une stratégie aléatoire le joueur ne gagnera pas(peu importe la profondeur de l'arbre)
				print(col)
				if is_valid_location(board, col):
					row = get_next_open_row(board, col)
